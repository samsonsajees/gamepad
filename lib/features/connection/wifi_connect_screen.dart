...
class _WifiConnectScreenState extends ConsumerState<WifiConnectScreen> {
  ...
  final _portCtrl  = TextEditingController(text: '${AppConstants.udpPort}');
      ...
      child: Column(
        ...
        children: [
          const Row(children: [
            ...
            SizedBox(width: 8),
            Text('WIFI SETUP', style: TextStyle(
              fontFamily: 'monospace', color: AppTheme.blue,
          ...
          for (final e in const [
            ...
            ('2', 'Launch gamepad_server.exe on Windows'),
            ('3', 'Click the WiFi tab in the Windows host app'),
            ('4', 'Scan the QR code shown there,\n    or enter the IP + token manually'),
          ])
            _step(e.$1, e.$2),
        ],
      ),
  ...
  Widget _manualForm() {
    return Column(
      children: [
        Row(children: [
          Expanded(flex: 3, child: _field(_ipCtrl, 'PC IP ADDRESS', '192.168.1.x')),
          const SizedBox(width: 10),
          Expanded(child: _field(_portCtrl, 'PORT', '5001',
              type: TextInputType.number)),
        ]),
        const SizedBox(height: 12),
        _field(_tokenCtrl, 'SESSION TOKEN', 'shown in Windows app',
            type: TextInputType.number),
        const SizedBox(height: 16),
        SizedBox(
            ...
            child: Material(
              ...
              child: InkWell(
                ...
                child: Center(child: _isLoading
                  ...
                  : const Text('CONNECT VIA WIFI', style: TextStyle(
  ...
  Future<void> _connectWifi() async {
    ...
    final port  = int.tryParse(_portCtrl.text.trim())  ?? AppConstants.udpPort;
